...
@param {Object} config Configures the authorizer. `config` is passed down to [`pub-keystore`](https://github.com/davedoesdev/pub-keystore#moduleexportsconfig-cb), [`jose`](https://github.com/panva/jose/blob/master/docs/README.md#jwtverifytoken-keyorstore-options) and [`webauthn4js`](https://rawgit-now.davedoesdev.now.sh/davedoesdev/webauthn4js/master/docs/modules/webauthn4js.html#exports). The following extra properties are supported:
...
- `{WebAuthn4JS} [webAuthn]` If you have a pre-existing [`WebAuthn4JS`](https://rawgit-now.davedoesdev.now.sh/davedoesdev/webauthn4js/master/docs/interfaces/webauthn4js.webauthn4js-1.html) instance, pass it here. The default is to make a new one by calling [`webauthn4js`](https://rawgit-now.davedoesdev.now.sh/davedoesdev/webauthn4js/master/docs/modules/webauthn4js.html#exports).
...
- `{AuthorizeJWT} authz` The `AuthorizeJWT` object. As well as `AuthorizeJWT`'s prototype methods, it has the following properties:
  ...
  - `{WebAuthn4JS} [webAuthn]` The [`WebAuthn4JS`](https://rawgit-now.davedoesdev.now.sh/davedoesdev/webauthn4js/master/docs/interfaces/webauthn4js.webauthn4js-1.html) object being used to verify assertions, if `WEBAUTHN_MODE` is truthy.
*/
...
- If `config.WEBAUTHN_MODE` _was_ passed truthy to [`module.exports`](#moduleexportsconfig-cb) then `authz_token` must be a [Web Authentication](https://www.w3.org/TR/webauthn/) assertion. It must either be an object with the following properties or a string of the form `issuer_id.id.clientDataJSON.authenticatorData.signature.userHandle` - i.e. `issuer_id` and the properties of `car.response` (both described below) separated by a period. In the latter case, the remaining `opts` property is obtained by calling `config.complete_webauthn_token` (see [`module.exports`](#moduleexportsconfig-cb)).
  ...
  - `{String} issuer_id` This is used to retrieve a [`User`](https://rawgit-now.davedoesdev.now.sh/davedoesdev/webauthn4js/master/docs/interfaces/webauthn4js.user.html) from `AuthorizeJWT`'s key store using [`PubKeyStore.prototype_get_pub_key_by_issuer_id`](https://github.com/davedoesdev/pub-keystore#pubkeystoreprototypeget_pub_key_by_issuer_idissuer_id-cb).
    - If the retrieved value has a `user` property then that is used as the user otherwise the retrieved value itself is used.
  - `{(`[`PublicKeyCredentialRequestOptions`](https://rawgit-now.davedoesdev.now.sh/davedoesdev/webauthn4js/master/docs/interfaces/webauthn4js.publickeycredentialrequestoptions.html)` => `[`PublicKeyCredentialRequestOptions`](https://rawgit-now.davedoesdev.now.sh/davedoesdev/webauthn4js/master/docs/interfaces/webauthn4js.publickeycredentialrequestoptions.html)`)}[]` [opts] Optional list of functions which were used to modify the login requirements when producing `car`.
  - `{`[`CredentialAssertionResponse`](https://rawgit-now.davedoesdev.now.sh/davedoesdev/webauthn4js/master/docs/interfaces/webauthn4js.credentialassertionresponse.html)`} car` The credential assertion result that was generated by the authenticator in the browser. It must contain an _unsigned_ JWT in its client data.
...
- `{`[`Credential`](https://rawgit-now.davedoesdev.now.sh/davedoesdev/webauthn4js/master/docs/interfaces/webauthn4js.credential.html)`} [credential]` If `config.WEBAUTHN_MODE` was passed truthy to [`module.exports`](#moduleexportsconfig-cb) then this contains the validated credential, plus the issuer ID in the `issuer_id` property and the [`User`](https://rawgit-now.davedoesdev.now.sh/davedoesdev/webauthn4js/master/docs/interfaces/webauthn4js.user.html) in the `user` property.
*/
        ...
        ths.keystore.get_pub_key_by_issuer_id(assertion_obj.issuer_id, function (err, user, uri, rev)
        {
            ...
            (async function ()
            {
                ...
                try
                {
                    ...
                    // are client-generated and -signed data which mean something to the app.
