...
@param {String|JWT} authz_token The token to authorize.
...
- If `config.WEBAUTHN_MODE` _was_ passed to `module.exports` then `authz_token` must be a [Web Authentication](https://www.w3.org/TR/webauthn/) assertion. It must have the following properties:
  ...
  - `{String(base64)|ArrayBuffer|null} expected_user_handle` The expected [user handle](https://www.w3.org/TR/webauthn/#user-handle) identifying the user for which the assertion was generated. Not all authenticators can store user handles, in which case this may be `null`.
  - `{Object} assertion` The assertion result that was generated by the authenticator in the browser. It must have the following properties:
    ...
    - `{String(base64)|ArrayBuffer} id|rawId` The WebAuthn credential identifier returned by the authenticator in the browser. This is not used to retrieve the public key.
...
It must have the following properties:
      ...
      - `{String(base64)|ArrayBuffer clientDataJSON` The [JSON-encoded data](https://www.w3.org/TR/webauthn/#sec-client-data) which was used by the browser to generate the assertion. The `challenge` property of the encoded data must contain an a base64-encoded _unsigned_ JWT containing the claims being made.
      - `{String(base64)|ArrayBuffer authenticatorData` The [bindings](https://www.w3.org/TR/webauthn/#sec-authenticator-data) made by the authenticator in the browser.
      - `{String(base64)|ArrayBuffer signature` The [signature](https://www.w3.org/TR/webauthn/#dom-authenticatorassertionresponse-signature) returned by the authenticator, signed using the private key which corresponds to the public key retrieved using `issuer_id`.
      - `{String(base64)|ArrayBuffer|null} userHandle` The [user handle](https://www.w3.org/TR/webauthn/#dom-authenticatorassertionresponse-userhandle) returned by the authenticator in the browser, or `null` if the authenticator didn't return one.
